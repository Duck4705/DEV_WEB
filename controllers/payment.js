...
exports.getTransactionStep1 = async (req, res) => {
    ...
    try {
        // First check if this is a temporary booking
        ...
        if (tempBooking) {
            ...
            return res.render('Transaction_Step1', {
                bookingId,
        ...
        // If not a temporary booking, check in database
        db.query(`
            SELECT dv.TongTien, 
            ...
            WHERE dv.ID_DV = ?
            GROUP BY dv.ID_DV`, [bookingId],
            (err, results) => {
                if (err || results.length === 0) {
                    console.error('Database error:', err);
                    return res.status(500).send('Không tìm thấy thông tin đặt vé');
                }
        ...
        console.error('Error in getTransactionStep1:', error);
        res.status(500).send('Đã xảy ra lỗi khi xử lý đơn đặt vé');
    }
};
...
exports.postTransactionStep2 = async (req, res) => {
    ...
    try {
        ...
        if (payment_method === 'vnpay') {
            // Following VNPay's exact specifications
            ...
            vnp_Params['vnp_OrderType'] = '170000'; // Entertainment category
        ...
        res.redirect(`/payment/transaction_step3?bookingId=${bookingId}&code=00`);
    } catch (error) {
