...
// Hàm trợ giúp để chuyển đổi ID_G thành định dạng ghế như A01, B02, etc.
function convertIDGToSeatId(idG) {
    // Trích xuất hàng và số từ định dạng ID_G
    // Giả sử định dạng ID_G là G1, G2, etc.
    ...
    // 1. Nếu ID_G đã ở định dạng như "A01" hoặc "B02"
    if (/^[A-Z]\d{2}$/.test(idG)) {
        return idG;
    }
    
    // 2. Nếu ID_G ở định dạng "G1", "G2", etc.
    ...
    if (match) {
        ...
        // Ánh xạ chỉ số hàng thành chữ cái (0 -> A, 1 -> B, etc.)
        const row = String.fromCharCode(65 + rowIndex); // 65 là ASCII cho 'A'
        
        // Định dạng seatId thành "A01", "B02", etc.
                ...
                if (!screenings[currentScreening]) {
                    ...
                    db.query(bookedSeatsQuery, [currentScreening], (err, results) => {
                        if (err) {
                            ...
                            // Đánh dấu ghế đã đặt
                            results.forEach(row => {
                                // Tạo ID ghế dựa trên mẫu A01, B02, etc.
                                // ID ghế thường được tạo từ hàng + số
                                // Chuyển đổi row.ID_G thành định dạng ID ghế
    ...
    if (screening.seats[seatId].status === 'available') {
        ...
        seatTimeouts[`${screeningId}-${seatId}`] = setTimeout(() => {
            if (screening.seats[seatId].status === 'selected' && 
                ...
                // Thông báo cho người dùng rằng thời gian giữ ghế đã hết
                ws.send(JSON.stringify({
        ...
        // Gửi thông báo cho tất cả các client ngay lập tức
        broadcastSeatUpdate(screeningId, seatId);
        ...
        // Cho phép người dùng hủy chọn lựa của chính mình
        screening.seats[seatId].status = 'available';
        screening.seats[seatId].selectedBy = null;
        screening.seats[seatId].selectedAt = null;
...
//Tạo một map để lưu trữ các đơn đặt vé tạm thời
const temporaryBookings = new Map();

// Xuất temporaryBookings để sử dụng trong các controller khác
